	...
	public function getProductContent( $productIds, $tableSettings, $preview = true, &$page = array() ) {
		...
		foreach ($orders as $i => $column) {
			switch ($column['main_slug']) {
				case 'thumbnail':
					$mobileStyles = '';
					if (isset($settings['responsive_mode']) && wp_is_mobile() && 'disable' !== $settings['responsive_mode']) {
			...
			foreach ($orders as $column) {
				switch ($column['main_slug']) {
					...
					case 'description':
						...
						if ($stripDescription) {
							$postContent = strip_tags($existMB ? mb_strimwidth($postContent, 0, $stripSize, '...') : $this->truncateWordwrap($postContent, $stripSize));
						}
						if ($displayDescriptionPopup && ( $frontend || $preview )) {
					...
					case 'short_description':
						...
						if ($stripDescriptionShort) {
							$postContent = strip_tags($existMB ? mb_strimwidth($postContent, 0, $stripSizeShort, '...') : $this->truncateWordwrap($postContent, $stripSizeShort));
						}
					...
					case 'add_to_cart':
						...
						if ($_product->get_stock_status() == 'outofstock') {
							...
							if ($frontend) {
								...
								if ( $isVariable && ! $hideVariationAttr ) {
									...
									foreach ($_product->get_available_variations() as $variation) {
										if ($variation['variation_is_visible']) {
											...
											$variationQuantity = isset($variation['max_qty']) ? ' data-quantity="' . esc_attr($variation['max_qty']) . '"' : ' data-quantity="' . esc_attr(__('n/a', 'woo-product-tables')) . '"';
								...
								$id = !empty($varInStock) ? $varInStock : $id;
	...
	public function generateTableHtml( $listPost, $frontend, $settings, $withHeader = true ) {
		...
		if ( $isPro ) {
			foreach ( $this->orderColumns as $column ) {
				if ( array_key_exists( 'description_popup', $column ) ) {
					if ( $column['description_popup'] ) {
