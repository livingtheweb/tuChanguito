		...
		if (empty($this->_options)) {
			...
			$this->_options = DispatcherWtbp::applyFilters('optionsDefine', array(
				'general' => array(
					...
					'opts' => array(
						'send_stats' => array('label' => esc_html__('Send usage statistic', 'woo-product-tables'), 'desc' => esc_html__('Send information about what plugin options you prefer to use, this will help us make our solution better for You.', 'woo-product-tables'), 'def' => '0', 'html' => 'checkboxHiddenVal'),
						'accent_neutralise' => array('label' => __('Accent neutralise for searching', 'woo-product-tables'), 'desc' => __('When searching a table with accented characters, it can be frustrating to have an input such as `Zurich` not match `Zürich` or `étoile` not match `etoile` in the table. Turn on if you want when searching replace accented characters with unaccented counterparts.', 'woo-product-tables'), 'def' => '0', 'html' => 'checkboxHiddenVal'),
					),
				),
			));
